...
int oqs_item_sign_generic(EVP_MD_CTX *ctx, const ASN1_ITEM *it, void *asn,
                          X509_ALGOR *alg1, X509_ALGOR *alg2,                   
                          ASN1_BIT_STRING *str, int NID_ALG) {
    ...
    const EVP_MD * oqs_md = EVP_MD_CTX_md(oqs_key->digest);
      // Requested Digest

    if (oqs_md != NULL && oqs_md != EVP_md_null()) {
      // Gets the Signature ID
      if (!OBJ_find_sigid_by_algs(&sigid, EVP_MD_CTX_type(oqs_key->digest), NID_ALG)) {
        fprintf(stderr, "ERROR: Cannot find the algorithm/hash combination, aborting!\n\n");
        // No Hash combination
        ECerr(EC_F_PKEY_OQS_DIGESTSIGN, EC_R_NO_SUCH_OQS_ALGORITHM);
        return -2;
      }
    }

    /* Set algorithm identifier */
    X509_ALGOR_set0(alg1, OBJ_nid2obj(sigid), V_ASN1_UNDEF, NULL);
    if (alg2 != NULL)
        X509_ALGOR_set0(alg2, OBJ_nid2obj(sigid), V_ASN1_UNDEF, NULL);
...
{                                                                              \
  int ossl_ret = 0;                                                            \
  ossl_ret = oqs_item_sign_generic(ctx, it, asn, alg1, alg2, str, NID_ALG);    \
  return ossl_ret;                                                             \
    /* Set algorithm identifier */                                             \
...
{                                                                                \
    fprintf(stderr, "DEBUG: oqs_sig_info_set (alg = %d)\n", NID_ALG);            \
                               ...
                               size_t tbslen)
{
    ...
    int do_hash_and_sign = EVP_MD_CTX_md(oqs_key->digest) != NULL ? 1 : 0;
    const EVP_MD *classical_md = NULL;
    unsigned int digest_len = 0;
    unsigned char digest[EVP_MAX_MD_SIZE]; /* init with max length */

    // fprintf(stderr, "[%s:%s] DEBUG: pkey_oqs_digestsign() called\n", __FILE__, __func__) && fflush(stderr);
    // fprintf(stderr, "[%s:%s] DEBUG: Do the Hash-N-Sign ? %d \n", __FILE__, __func__, do_hash_and_sign) && fflush(stderr);
    ...
    if (do_hash_and_sign) {
      ...
      /* Classical MD - Appropriately Set via CTRL */
      classical_md = EVP_MD_CTX_md(oqs_key->digest);
...
static int pkey_oqs_ctrl(EVP_PKEY_CTX *ctx, int type, int p1, void *p2)
{
    ...
    switch (type) {
      ...
      case EVP_PKEY_CTRL_MD:
          /* Free current */
          if (oqs_key->digest) EVP_MD_CTX_destroy(oqs_key->digest);
          oqs_key->digest = NULL;
          /* NULL allowed as digest */
          if (p2 == NULL || p2 == EVP_md_null) return 1;
          /* Create the new CTX */
          if ((oqs_key->digest = EVP_MD_CTX_create()) == NULL) return 0;
          /* Initialize with the requested hash */
...
static int oqs_int_update(EVP_MD_CTX *ctx, const void *data, size_t count)
{
    ...
    if (oqs_key->digest == NULL) {
        fprintf(stderr, "[%s:%s] DEBUG: MD is NULL, creating a new one (hash-sign)\n", __FILE__, __func__);
        ...
        if (EVP_DigestInit_ex(oqs_key->digest, EVP_sha512(), NULL) <= 0) {
          fprintf(stderr, "DEBUG: Cannot initialize OQS with SHA512\n");
       		return 0;
       	}
    }

    if (EVP_DigestUpdate(oqs_key->digest, data, count)<=0) {
      fprintf(stderr, "DEBUG: cannot update the MD\n");
	    return 0;
    }
...
static int pkey_oqs_verifyctx(EVP_PKEY_CTX *ctx, const unsigned char *sig, int siglen,
  ...
  fprintf(stderr, "DEBUG: pkey_oqs_verifyctx() called with sig = %p\n", sig) && fflush(stderr);
