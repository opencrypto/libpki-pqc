                               ...
                               size_t tbslen)
{
  ...
  */
  ...
  /*
  // Calculates the Digest to use - differently from the OQS case
  // [OpenCA] we sign the digest with both PQ and classic to make
  // sure the same data is signed (equivalent keys require equivalent
  // data to be signed)
  if (oqs_key->digest == NULL) {
    // If we do not have a digest algorithm set, the one
    // that carries the appropriate security bits is selected
    if (EVP_MD_CTX_md(ctx) == NULL) {
      ...
      // We have an MD set, but no digest. We recreate it
      if ((oqs_key->digest = EVP_MD_CTX_create()) == NULL) {
        return 0;
      }

      if (EVP_DigestInit_ex(oqs_key->digest, EVP_MD_CTX_md(ctx), NULL) <= 0) {
        return 0;
      }

      if (EVP_DigestUpdate(oqs_key->digest, tbs, tbslen) <=0 ) {
        return 0;
      }

      if (EVP_DigestFinal(oqs_key->digest, &digest, &digest_len) <= 0) {
        return 0;
      }
...
static int pkey_oqs_signctx(EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen, EVP_MD_CTX *mctx) {
    ...
    if (sig != NULL) {
      ...
      // Checks for the MD CTX
      if (!mctx || EVP_MD_CTX_size(mctx)) {
        fprintf(stderr, "DEBUG: ERROR: Missing Digest (mctx=%p, size=%d)",
          mctx, mctx == NULL ? 0 : EVP_MD_CTX_size(mctx));
		    return 0;
      }
    ...
    */
    ...
    if (sig != NULL) {
        ...
        fprintf(stderr, "DEBUG: ERROR: Missing Digest Calculation")
		    return 0;
	    }
